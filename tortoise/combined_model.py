...
def do_spectrogram_diffusion(
    ...
    with torch.no_grad():
        ...
        precomputed_embeddings = diffusion_model.timestep_independent(
            latents.to(device), conditioning_latents.to(device), output_seq_len, False
        )

        noise = torch.randn(output_shape, device=device) * temperature
...
class TTSModel(nn.Module):
    ...
    def from_pretrained(self, checkpoint_path):
        ...
        self.autoregressive.load_state_dict(ckpt["autoregressive"], strict=False)
        self.autoregressive.post_init_gpt2_config(
            use_deepspeed=self.use_deepspeed, kv_cache=self.kv_cache, half=self.half
        )
        self.autoregressive.to(self.device)
        self.clvp.load_state_dict(ckpt["clvp2"])
        self.clvp.to(self.device)
        self.diffusion.load_state_dict(ckpt["diffusion_decoder"])
        self.diffusion.to(self.device)
        self.vocoder.load_state_dict(ckpt["generator"])
        self.vocoder.eval(inference=True)
        self.vocoder.to(self.device)
        self.rlg_auto.load_state_dict(ckpt["rlg_auto"])
    ...
    def get_random_conditioning_latents(self):
        with torch.no_grad():
            return self.rlg_auto(
                torch.tensor([0.0]).to(self.device)
            ), self.rlg_diffusion(torch.tensor([0.0]).to(self.device))
